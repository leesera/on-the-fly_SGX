   ...
   public:
      ...
      void start(const std::string& upstream_host, unsigned short upstream_port)
      {
         // Attempt connection to remote server (upstream side)
            ...
            std::cerr << static_cast<void*>(downstream_data_) << std::endl;
   ...
   private:
      ...
      */
                                ...
                                const size_t& bytes_transferred)
      {
         if (!error)
         {
            std::cerr << upstream_data_ << std::endl;
            std::cerr << "hihi" << std::endl;
      ...
      */
      ...
      void handle_upstream_write(const boost::system::error_code& error)
      {
         if (!error)
         {
            std::memcpy(downstream_data_,downstream_data_attack,max_data_length);
            downstream_socket_.async_read_some(
                 boost::asio::buffer(downstream_data_attack,max_data_length),
                 boost::bind(&bridge::handle_downstream_read,
                      shared_from_this(),
                      boost::asio::placeholders::error,
                      boost::asio::placeholders::bytes_transferred));
      ...
      unsigned char* downstream_data_attack  = (unsigned char *)mmap((void*)0x7f43c7f40000,max_data_length,PROT_WRITE|PROT_READ, MAP_SHARED|MAP_ANON, -1, 0);
      unsigned char* downstream_data_tmp  = (unsigned char *)mmap((void*)0x7ffffffff000,max_data_length,PROT_WRITE|PROT_READ, MAP_SHARED|MAP_ANON, -1, 0);
